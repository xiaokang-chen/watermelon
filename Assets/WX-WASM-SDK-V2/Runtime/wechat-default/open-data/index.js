...
import { getFriendScoreRankXML, getFriendFeelingsRankXML } from './render/tpls/friendRank';
...
const RankKeyType = {
    FRIENDS_SCORE_RANK: 'ScoreRank',
    FRIENDS_FEELINGS_RANK: 'FeelingsRank',
};
/**
 * 绑定邀请好友事件
...
async function renderFriendsRank(rankKey) {
    showLoading();
    try {
        const data = await getFriendRankData(rankKey ?? RANK_KEY);
        if (!data.length) {
            renderTips('暂无好友数据');
            return;
        }
        if (RankKeyType.FRIENDS_FEELINGS_RANK == rankKey) {
            LayoutWithTplAndStyle(getFriendFeelingsRankXML({
                data,
            }), getFriendRankStyle({
                width: sharedCanvas.width,
                height: sharedCanvas.height,
            ...
            LayoutWithTplAndStyle(getFriendScoreRankXML({
                data,
            }), getFriendRankStyle({
                width: sharedCanvas.width,
                height: sharedCanvas.height,
        ...
        console.error('renderFriendsRank error', e, rankKey);
        renderTips('请进入设置页允许获取微信朋友信息');
    }
}
// 渲染群排行榜
async function renderGroupFriendsRank(rankKey, shareTicket) {
    showLoading();
    try {
        const data = await getGroupFriendsRankData(shareTicket, rankKey ?? RANK_KEY);
        if (!data.length) {
    ...
    wx.onMessage((data) => {
        ...
        switch (data.type) {
            ...
            case MessageType.SHOW_FRIENDS_RANK:
                renderFriendsRank(data.rankKey);
                break;
            case MessageType.SHOW_GROUP_FRIENDS_RANK:
                renderGroupFriendsRank(data.rankKey ?? RANK_KEY, data.shareTicket);
                break;
            case MessageType.SET_USER_RECORD:
                setUserRecord(data.rankKey ?? RANK_KEY, data.score);
                break;
            default:
